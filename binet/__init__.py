"""Module for building, visualizing and calculating all the properties of bipartite networks."""
from classes import mcp,tnet,BiGraph
from functions import build_connected,build_html,CalculateComplexity,calculateRCA,df_interp,order_columns,flatten,compare_nets
from functions_dl import WDI_get,trade_data
from functions_gt import get_pos